public class NumberOfWaysToArrangeBalls {
    ...
    public static long totalWays(int a, int b, int c)
    {
        ...
        return f(a-1,b,c,0)+f(a,b-1,c,1)+f(a,b,c-1,2);
    }
    static long f(int a,int b,int c,int prev){
        ...
        long count = 0;
        if(prev==0)
            count+= f(a,b-1,c,1)+ f(a,b,c-1,2);
        if(prev==1)
            count+= f(a-1,b,c,0)+ f(a,b,c-1,2);
        if(prev==2)
            count+= f(a-1,b,c,0)+f(a,b-1,c,1);
